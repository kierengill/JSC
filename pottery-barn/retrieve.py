'''Scrapes on sale listings from Pottery Barn website'''
...
def RunScraper(url_list, url_names, index):
    ...
    products = soup.find_all('div', {'data-component': 'Shop-ProductCell'})
    ...
    for item in products:
        if item != None:
            count += 1
            sale = item.find('span', {'class': 'sale-price'})
            suggested = item.find('span', {'class': 'suggested-price'})
    ...
    fields = ['brand', 'category', 'regularPrice', 'salePrice',
        'salePercentOff']
