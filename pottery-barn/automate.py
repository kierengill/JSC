'''Automates running each script to track on sale Pottery Barn listings'''
...
# run all scripts to generate and import Pottery Barn on sale listings data
def ImportData():
    ...
    if test:
        ...
        print("Importing data into database: Home-Furnishing, collection: Pottery-Barn")
        os.system(f'mongoimport --uri mongodb+srv://{username}:{password}@projects.bcwtn.mongodb.net/Home-Furnishing --collection Pottery-Barn --type csv --file aggregated.csv --headerline')
        os.system('rm aggregated.csv')
        print(f'Finished importing aggregated.csv')
