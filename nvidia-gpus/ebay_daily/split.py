...
def SplitEbay3090New():
    df = pd.read_csv("ebay_3090_24_new.csv")
    lendf = len(df)
...
def SplitEbay3090Used():
    ...
    # write the split Ti data to a new file
    with open ('ebay_3090Ti_24_used.csv', 'w') as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames = FIELDS)
        writer.writeheader()
        writer.writerows(listings)
    
    # commit the edited csv that contains non-Ti listings to the original file
    df.to_csv("ebay_3090_24_used.csv", index=False)

# split 3070 new data into Ti and non-Ti files
def SplitEbay3070New():
    df = pd.read_csv("ebay_3070_8_new.csv")
    lendf = len(df)
    ...
    # write the split Ti data to a new file
    with open ('ebay_3070Ti_8_new.csv', 'w') as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames = FIELDS)
        writer.writeheader()
        writer.writerows(listings)

    # commit the edited csv that contains non-Ti listings to the original file
    df.to_csv("ebay_3070_8_new.csv", index=False)

# split 3070 used data into Ti and non-Ti files
def SplitEbay3070Used():
    df = pd.read_csv("ebay_3070_8_used.csv")
    lendf = len(df)
    ...
    # write the split Ti data to a new file
    with open ('ebay_3070Ti_8_used.csv', 'w') as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames = FIELDS)
        writer.writeheader()
        writer.writerows(listings)

    # commit the edited csv that contains non-Ti listings to the original file
    df.to_csv("ebay_3070_8_used.csv", index=False)
