'''Automates running each script to track on sale Williams Sonoma listings'''
...
# run the script above and import data into MongoDB
def ImportData():
    ...
    if test:
        ...
        os.system(f'rm aggregated.csv')
        print(f'Finished importing aggregated.csv')
    else:
        print("Importing data into database: Home-Furnishing, collection: Williams-Sonoma")
        os.system(f'mongoimport --uri mongodb+srv://{username}:{password}@projects.bcwtn.mongodb.net/Home-Furnishing --collection Williams-Sonoma --type csv --file aggregated.csv --headerline')
        os.system(f'rm aggregated.csv')
        print(f'Finished importing aggregated.csv')
